    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md bg-white rounded-3xl p-10 text-center shadow-lg border border-slate-100">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Vendor Registered</h2>
          <p className="text-slate-500 mb-8">Successfully registered as a vendor. Please login to continue.</p>
          <Link href="/login" className="w-full inline-block py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-sm">Sign In Now</Link>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 sm:p-12">
      <div className="w-full max-w-xl">
        <div className="text-center mb-10">
            ...
            <h2 className="text-3xl font-bold text-slate-900 mb-2">Join as a Vendor</h2>
            <p className="text-slate-500 text-sm">Create your business profile to start receiving leads.</p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-100 p-8 sm:p-12">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Business Email</label>
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className={`px-5 text-sm font-bold rounded-xl transition-all ${otpSent ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-900 text-white'}`}
                        >
                ...
                <AnimatePresence>
                    {otpSent && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-1.5">
                            <label className="text-xs font-bold text-emerald-600 uppercase tracking-wider ml-1">Verification Code</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-11 pr-4 py-3.5 bg-emerald-50/10 border border-emerald-200 rounded-xl outline-none focus:border-emerald-500 text-lg font-bold tracking-[8px] text-center text-slate-700 transition-all"
                                    placeholder="••••••"
                                    maxLength={6}
                                />
                            </div>
                ...
                <div className="pt-4">
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 shadow-sm shadow-blue-200"
                    >
                ...
                <div className="text-center pt-8 border-t border-slate-50">
                    <p className="text-sm text-slate-400 mb-1">Already have an account?</p>
                    <Link href="/login" className="text-sm font-bold text-blue-600 hover:underline">Sign in here</Link>
                </div>
            </form>
        </div>

        <div className="text-center mt-10">
            <p className="text-xs text-slate-400">Joining as a Buyer? Use our <Link href="/login" className="text-blue-600 font-bold hover:underline">Phone Login</Link> for instant access.</p>
        </div>
      </div>
    </div>
  );
