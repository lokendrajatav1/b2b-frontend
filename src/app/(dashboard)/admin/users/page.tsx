    ...
    <div className="space-y-8 animate-simple-fade pb-20 p-2 md:p-0">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-gray-100 max-w-7xl mx-auto">
        <div>
           ...
           <p className="text-gray-500 font-medium mt-1 text-sm">Review activity, manage permissions, and assign administrative roles.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
           <div className="flex bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              ...
              <select 
                 ...
                 className="px-3 py-2 text-sm font-medium text-gray-600 bg-transparent outline-none cursor-pointer hover:bg-gray-50 transition-colors w-32"
              >
           ...
           <div className="relative">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium outline-none focus:border-blue-500 transition-all w-64 shadow-sm"
              />
           </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm h-[calc(100vh-240px)] flex flex-col">
            ...
            <table className="w-full text-left border-collapse relative min-w-[800px]">
              <thead className="sticky top-0 z-20 bg-gray-50/95 backdrop-blur-md">
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Access Role</th>
              ...
              {loading ? (
                ...
                filteredUsers.map((user) => (
                  <tr key={user.id} className="group hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-4">
                         <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-widest uppercase border ${
                            user.role === 'ADMIN' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 
                            user.role === 'VENDOR' ? 'bg-blue-50 text-blue-700 border-blue-100' : 
                            user.role === 'SUBADMIN' ? 'bg-purple-50 text-purple-700 border-purple-100' : 
                    ...
                    <td className="px-6 py-4">
                      <p className="text-xs font-semibold text-gray-500">{new Date(user.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2 transition-opacity duration-300">
                         {user.role === 'VENDOR' && user.vendor && (
                            ...
                            >
                              ...
                              {(user.vendor.manualBoost || 0) > 0 && (
                                <span className="absolute -top-1.5 -right-1.5 flex h-4 min-w-4 px-1 items-center justify-center rounded-full bg-amber-600 text-[9px] font-black text-white ring-2 ring-white">
                ...
                <tr>
                  <td colSpan={5} className="py-24 text-center">
                    ...
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Workspace Isolated</p>
                    <p className="text-xs font-medium text-gray-400 mt-1">No members match the current search filters.</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleUpdateUser} className="p-6 space-y-5">
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Member Name</label>
                  <input type="text" disabled value={(selectedUser.name && !selectedUser.name.includes('_') ? selectedUser.name : selectedUser.phone) || 'Anonymous User'} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-gray-500 cursor-not-allowed" />
                </div>
                
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Platform Role</label>
                  <select 
                    value={selectedUser.role}
                    onChange={(e) => setSelectedUser({...selectedUser, role: e.target.value})}
                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-700 outline-none focus:border-blue-500 transition-all uppercase tracking-widest"
                  >
                    <option value="BUYER">Buyer (Standard User)</option>
                    <option value="VENDOR">Vendor</option>
                    <option value="SUBADMIN">Sub Admin</option>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900">Account Access</h4>
                    <p className="text-xs font-medium text-gray-500 mt-0.5">Allow login and platform usage</p>
                  </div>
                  <button 
                    type="button"
                ...
                <div className="pt-2 flex items-center justify-end gap-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-sm font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-colors">
                    Cancel
                  </button>
                  <button type="submit" disabled={updatingId === 'modal'} className="px-5 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-500/30 hover:bg-blue-700 hover:shadow-blue-500/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
        ...
        {isBoostModalOpen && boostConfig && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white/95 backdrop-blur-xl rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] w-full max-w-md overflow-hidden border border-white/50"
            >
              <div className={`px-8 py-7 relative overflow-hidden transition-colors duration-500 ${
                parseFloat(boostConfig.score) <= 0 ? 'bg-gray-100 text-gray-900' : 
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-black tracking-tight">Boost Visibility</h3>
                      <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest mt-0.5">Admin Influence Matrix</p>
                    </div>
                  </div>
              ...
              <form onSubmit={handleBoost} className="p-8 space-y-8">
                <div className="relative group">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Multiplier Intensity</span>
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tight transition-colors ${
                      parseFloat(boostConfig.score) <= 0 ? 'bg-gray-100 text-gray-500' : 
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="relative min-w-[100px]">
                      <input type="number" step="0.1" min="0.0" max="100.0" value={boostConfig.score} onChange={(e) => setBoostConfig({...boostConfig, score: e.target.value})} className={`w-full bg-gray-50 border-2 rounded-2xl px-4 py-4 text-3xl font-black transition-all text-center focus:ring-4 focus:ring-amber-500/10 ${parseFloat(boostConfig.score) <= 0 ? 'border-gray-100 text-gray-400 focus:border-gray-300' : parseFloat(boostConfig.score) < 10 ? 'border-amber-100 text-amber-600 focus:border-amber-400' : parseFloat(boostConfig.score) < 25 ? 'border-orange-100 text-orange-600 focus:border-orange-400' : 'border-purple-100 text-purple-600 focus:border-purple-400'}`} />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                  <div className="flex items-center gap-3 mb-2">
                    <Target className={`w-4 h-4 ${parseFloat(boostConfig.score) > 0 ? 'text-amber-500' : 'text-gray-400'}`} />
                    <span className="text-xs font-bold text-gray-900">Experience Impact Preview</span>
                  </div>
                ...
                <div className="pt-2 flex flex-col gap-3">
                  <button type="submit" disabled={updatingId === 'boost'} className={`w-full py-4 text-sm font-black rounded-2xl shadow-2xl transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 ${parseFloat(boostConfig.score) <= 0 ? 'bg-gray-900 text-white shadow-gray-200' : parseFloat(boostConfig.score) < 10 ? 'bg-amber-500 text-white shadow-amber-500/20 hover:bg-amber-600' : parseFloat(boostConfig.score) < 25 ? 'bg-orange-500 text-white shadow-orange-500/20 hover:bg-orange-600' : 'bg-purple-600 text-white shadow-purple-600/20 hover:bg-purple-700'}`}>
                    {updatingId === 'boost' ? <RefreshCcw className="w-5 h-5 animate-spin" /> : <><Zap className="w-5 h-5" />Apply Intensity Level</>}
                  </button>
                  <button type="button" onClick={() => setIsBoostModalOpen(false)} className="w-full py-3 text-sm font-bold text-gray-400 hover:text-gray-600 transition-colors">Dismiss Editor</button>
                </div>
              </form>
            </motion.div>
          </div>
            ...
            >
               <div className="p-8 text-center">
                  ...
                  <p className="text-gray-500 font-medium text-sm leading-relaxed mb-8">
                    Are you sure you want to delete <span className="text-gray-900 font-bold">{userToDelete.name || userToDelete.email}</span>? 
                  ...
                  <div className="flex flex-col gap-3">
                    <button 
                      ...
                      className="w-full py-4 bg-red-600 text-white text-sm font-bold rounded-2xl shadow-xl shadow-red-500/20 hover:bg-red-700 hover:shadow-red-500/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                    ...
                    <button 
                      ...
                      disabled={updatingId === userToDelete.id}
                      className="w-full py-4 text-sm font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-50 rounded-2xl transition-all"
                    >
                      Keep Account
                    </button>
                  </div>
