...
export default function AdminVendorApprovals() {
  ...
  const fetchVendors = async () => {
    try {
      ...
      // Admin API returns directly the array or an object depending on version
      // Super Admin API returns { vendors: [], total: 0 }
      if (data.data?.vendors) {
        setVendors(data.data.vendors);
        setTotal(data.data.total || data.data.vendors.length);
      } else {
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex flex-col gap-3 pb-6 border-b border-gray-100 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-5">
           ...
           <div>
              <h1 className="text-xl font-semibold text-slate-900">Hub Partner Registry</h1>
              <p className="text-sm text-gray-600 font-normal mt-1">Manage business on-boarding for your assigned hub.</p>
           </div>
        </div>
         ...
         <div className="bg-white border border-gray-100 rounded-xl overflow-hidden flex flex-col shadow-sm">
                 ...
                 <tr>
                    <td colSpan={5} className="py-32">
                       <div className="flex flex-col items-center text-center">
                          ...
                          <h3 className="text-base font-bold text-slate-900 mb-1">Awaiting hub partner screening.</h3>
            ...
            <div className="px-8 py-6 bg-slate-50/30 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-6">
               <p className="text-xs font-semibold text-slate-600 uppercase tracking-tight">
                  Showing <span className="text-slate-900 font-bold">{Math.min(((page - 1) * limit) + 1, total)}</span> to <span className="text-slate-900 font-bold">{Math.min(page * limit, total)}</span> of <span className="text-slate-900 font-bold">{total}</span> partners
               </p>
               ...
               <div className="flex items-center gap-6">
                  ...
                  >
                     ...
                     <option value={25}>25 partners</option>
                     <option value={50}>50 partners</option>
                  </select>
        ...
        {isModalOpen && selectedVendor && (
          <>
            ...
            <motion.div 
               ...
               className="fixed right-0 top-0 h-screen w-full max-w-2xl bg-[#F8FAFC]  z-[110] flex flex-col overflow-hidden shadow-2xl"
            >
               ...
               <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     ...
                     <div className="space-y-6">
                        <div className="space-y-4">
                           <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Contact Details</h4>
                           <div className="space-y-3">
                              ...
                              <InfoRow icon={<MapPin className="text-rose-500" />} label="Location" value={selectedVendor.address || 'Location details pending'} />
                           </div>
                        </div>

                        <div className="space-y-4">
                           <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Social Assets</h4>
                     ...
                     <div className="space-y-6">
                        <div className="space-y-4">
                           <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Verifiable ID</h4>
                           <div className="space-y-3">
                        ...
                        <div className="space-y-4">
                           <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Primary Categories</h4>
                           <div className="flex flex-wrap gap-2">
                              {selectedVendor.categories?.map((c: any) => (
                                 <span key={c.id} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg text-[10px] font-bold uppercase">{c.name}</span>
                              )) || <span className="text-sm font-bold text-slate-600 italic">No category linked</span>}
                           </div>
                        </div>
                     </div>
                  </div>
                  ...
                  {/* Description */}
                  <div className="space-y-4">
                     <h4 className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Business Statement</h4>
                  ...
                  <div className="pt-8 border-t border-gray-100 flex items-center gap-3 shrink-0">
                     ...
                     >
                        ...
                        {processingId === selectedVendor.id ? 'Wait...' : 'Authorize Partner'}
                     </button>
                     <button 
