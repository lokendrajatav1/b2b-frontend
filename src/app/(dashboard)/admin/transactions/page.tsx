    ...
    <div className="space-y-8 animate-simple-fade pb-20 p-2 md:p-0">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-gray-100 max-w-7xl mx-auto">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 text-sm">Audit platform revenue and vendor subscriptions.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
           ...
           <button onClick={fetchTransactions} className="p-2 bg-white border border-gray-200 rounded-xl text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-all shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
         {[
           { label: 'Total Volume', value: `₹${stats.totalRev.toLocaleString()}`, icon: Wallet, bg: 'bg-blue-50', text: 'text-blue-600' },
           { label: 'Total Transactions', value: stats.count, icon: CreditCard, bg: 'bg-indigo-50', text: 'text-indigo-600' },
           ...
           <div key={i} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between">
              <div>
                 ...
                 <h2 className="text-2xl font-bold text-gray-900 tracking-tight">{s.value}</h2>
              </div>
              ...
              <table className="w-full text-left">
                 <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-100">
                       ...
                       <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Date & Method</th>
                    ...
                    {loading && transactions.length === 0 ? (
                      ...
                      <tr key={t.id} className="hover:bg-gray-50 transition-colors group">
                         <td className="px-6 py-4">
                            <code className="text-xs font-medium text-gray-600 bg-gray-100 px-2.5 py-1 rounded inline-block">
                              {t.razorpayPaymentId || t.id.slice(0, 12)}
                            </code>
                         </td>
                         <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                               <div className="w-8 h-8 bg-blue-50 rounded flex items-center justify-center text-blue-600 font-semibold text-xs border border-blue-100">
                                  {t.vendor?.businessName?.charAt(0).toUpperCase() || 'V'}
                               </div>
                               <div>
                                  <p className="text-sm font-semibold text-gray-900">{t.vendor?.businessName || 'Anonymous'}</p>
                                  <p className="text-xs font-medium text-gray-500">{t.vendor?.ownerName}</p>
                               </div>
                            </div>
                         </td>
                         <td className="px-6 py-4">
                            <div className="flex flex-col">
                               ...
                               <span className="text-xs font-medium text-gray-400 ml-5 mt-0.5">Razorpay</span>
                            </div>
                         </td>
                         <td className="px-6 py-4">
                            <span className="text-base font-semibold text-gray-900">₹{t.amount}</span>
                         </td>
                         <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 rounded-md text-xs font-semibold uppercase tracking-wider border ${getStatusColor(t.status)}`}>
                               {t.status}
                            </span>
                         </td>
                      </tr>
