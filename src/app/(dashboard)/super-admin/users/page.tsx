    ...
    <div className="space-y-8 animate-simple-fade pb-20 p-2 md:p-0">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-gray-100 max-w-7xl mx-auto">
        <div>
           ...
           <p className="text-slate-700 font-medium mt-1 text-sm">Review activity, manage permissions, and assign administrative roles.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
           <div className="flex bg-white border border-gray-200 rounded-xl overflow-hidden ">
              ...
              <select 
                 ...
                 className="px-3 py-2 text-sm font-medium text-slate-800 bg-transparent outline-none cursor-pointer hover:bg-gray-50 transition-colors w-32"
              >
           ...
           <div className="relative">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium outline-none focus:border-blue-500 transition-all w-64 "
              />
           </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-xl border border-gray-200  h-[calc(100vh-240px)] flex flex-col">
            ...
            <table className="w-full text-left border-collapse relative min-w-[800px]">
              <thead className="sticky top-0 z-20 bg-gray-50/95 backdrop-blur-md">
                <tr className="border-b border-gray-100">
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase ">Profile</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase ">Email</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase ">Phone</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase ">Access Role</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase ">Joined On</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-700 uppercase  text-right">Actions</th>
                </tr>
              </thead>
              ...
              {loading ? (
                  ...
                  <tr key={user.id} className="group hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-slate-900 capitalize leading-none">
                               {user.name && isNaN(Number(user.name)) ? user.name : 'Guest Member'}
                            </p>
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <span className={`w-1.5 h-1.5 rounded-full ${user.isActive ? 'bg-emerald-500 (16,185,129,0.4)]' : 'bg-red-500'}`}></span>
                            <span className="text-sm font-semibold text-slate-700 uppercase  leading-none">
                    ...
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-4">
                         <div className={`px-2.5 py-1 rounded-lg text-sm font-semibold  uppercase border ${
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-semibold text-slate-700">{new Date(user.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2 transition-opacity duration-300">
                         {user.role === 'VENDOR' && user.vendor && (
                            ...
                            >
                              ...
                              {(user.vendor.manualBoost || 0) > 0 && (
                                <span className="absolute -top-1.5 -right-1.5 flex h-4 min-w-4 px-1 items-center justify-center rounded-full bg-amber-600 text-sm font-semibold text-white ring-2 ring-white">
                         ...
                         <button 
                           ...
                           title="Edit Access"
                           className="p-2 text-slate-700 hover:text-[#164e33] hover:bg-[#164e33]/5 rounded-xl transition-all duration-300"
                         >
                         ...
                         <button 
                           ...
                           className="p-2 text-slate-700 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-300"
                         >
                ...
                <tr>
                  <td colSpan={5} className="py-24 text-center">
                    ...
                    <p className="text-sm font-semibold text-slate-700 uppercase ">Workspace Isolated</p>
                    <p className="text-sm font-medium text-slate-700 mt-1">No members match the current search filters.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {isModalOpen && selectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white rounded-xl  w-full max-w-md overflow-hidden border border-gray-100"
            >
              ...
              <form onSubmit={handleUpdateUser} className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-800 outline-none focus:border-blue-500 transition-all uppercase "
                  >
                    <option value="BUYER">Buyer (Standard User)</option>
                    <option value="VENDOR">Vendor</option>
                    <option value="ADMIN">Admin</option>
                ...
                <div className="pt-2 flex items-center justify-end gap-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-sm font-semibold text-slate-700 hover:text-slate-800 hover:bg-gray-100 rounded-xl transition-colors">
                    Cancel
                  </button>
                  <button type="submit" disabled={updatingId === 'modal'} className="px-5 py-2.5 bg-[#164e33] text-white text-sm font-semibold rounded-xl   hover:bg-[#113f29] hover: transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
        ...
        {isBoostModalOpen && boostConfig && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white/95 backdrop-blur-xl rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-white/50"
            >
              ...
              <form onSubmit={handleBoost} className="p-8 space-y-8">
                <div className="relative group">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-sm font-semibold uppercase  transition-colors ${
                ...
                <div className="pt-2 flex flex-col gap-3">
                  <button type="submit" disabled={updatingId === 'boost'} className={`w-full py-4 text-sm font-semibold rounded-2xl  transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 ${parseFloat(boostConfig.score) <= 0 ? 'bg-gray-900 text-white ' : parseFloat(boostConfig.score) < 10 ? 'bg-amber-500 text-white  hover:bg-amber-600' : parseFloat(boostConfig.score) < 25 ? 'bg-orange-500 text-white  hover:bg-orange-600' : 'bg-purple-600 text-white  hover:bg-purple-700'}`}>
        ...
        {isDeleteModalOpen && userToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white rounded-xl  w-full max-w-sm overflow-hidden border border-gray-100"
            >
               <div className="p-8 text-center">
                  <div className="w-20 h-20 bg-red-50 text-red-600 rounded-[24px] flex items-center justify-center mx-auto mb-6  border border-red-100">
                  ...
                  <div className="flex flex-col gap-3">
                    <button 
                      ...
                      className="w-full py-4 bg-red-600 text-white text-sm font-semibold rounded-2xl   hover:bg-red-700 hover: transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                    ...
                    <button 
                      ...
                      disabled={updatingId === userToDelete.id}
                      className="w-full py-4 text-sm font-semibold text-slate-700 hover:text-slate-800 hover:bg-gray-50 rounded-2xl transition-all"
                    >
                      Keep Account
                    </button>
                  </div>
