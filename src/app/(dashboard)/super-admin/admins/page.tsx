...
export default function AdminManagement() {
  ...
  const fetchAdmins = async () => {
    try {
      setLoading(true);
      const res = await apiFetch('/admins'); // Using /admins route
      setAdmins(res.data || []);
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-simple-fade pb-20 p-2 md:p-0">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-gray-100 max-w-7xl mx-auto">
        <div>
           <h1 className="text-2xl font-semibold text-slate-900  flex items-center gap-3">
             Admin Management
             <div className="p-1.5 bg-[#164e33]/10 text-[#164e33] rounded-lg border border-[#164e33]/10">
        ...
        <div className="flex items-center gap-3">
           <button onClick={fetchAdmins} className="p-2.5 bg-white border border-gray-200 rounded-lg text-slate-700 hover:text-[#164e33] hover:bg-[#164e33]/5 transition-all ">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
             [1,2,3].map(i => (
                <div key={i} className="animate-pulse bg-white border border-gray-100 rounded-lg h-80 shadow-sm"></div>
             ))
          ) : admins.length > 0 ? (
             admins.map(admin => (
                  <motion.div 
                    layout
                    key={admin.id} 
                    className="bg-white border border-gray-100 rounded-lg p-6 hover:shadow-lg transition-all group relative flex flex-col h-full"
                  >
                     ...
                     <div className="flex items-center gap-4 mb-6">
                        <div className="relative shrink-0">
                           <div className="w-14 h-14 rounded-lg overflow-hidden border border-slate-100 bg-slate-50 flex items-center justify-center shadow-sm">
                              {admin.user?.avatar ? (
             ...
             <div className="col-span-full py-24 text-center border-4 border-dashed border-slate-100 rounded-lg bg-slate-50/50">
          ...
          >
             ...
             >
               ...
               <form onSubmit={handleSubmit} className="p-8 space-y-6">
                  {error && (
                     <div className="p-3 bg-rose-50 border border-rose-100 rounded-lg text-rose-600 text-sm font-semibold">
                        {error}
                     </div>
                  )}
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                          ...
                          <input 
                            ...
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:bg-white focus:border-[#164e33] text-sm font-medium transition-all disabled:opacity-50"
                        ...
                        {!editingId && (
                          <div className="space-y-1">
                            ...
                            <input 
                              ...
                              className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:bg-white focus:border-[#164e33] text-sm font-medium transition-all"
                        ...
                        <div className={`space-y-1 ${editingId ? 'sm:col-span-2' : ''}`}>
                          ...
                          <div className="relative">
                            <select 
                              ...
                              className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:bg-white focus:border-[#164e33] text-sm font-medium transition-all appearance-none cursor-pointer pr-10"
                            >
                     ...
                     <div className="space-y-3 pt-2">
                        ...
                        <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
                           {availablePermissions.map(perm => (
                             <label
                                key={perm.id}
                                className={`flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition-all ${formData.permissions.includes(perm.id) ? 'bg-[#164e33]/5 border-[#164e33]/20 ring-1 ring-[#164e33]/10' : 'bg-gray-50 border-gray-100 hover:bg-gray-100'}`}
                             >
                                <input 
                                   type="checkbox"
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t border-gray-50">
                     ...
                     <button 
                       type="submit" 
                       disabled={submitting}
                       className="px-8 py-2.5 bg-[#164e33] hover:bg-[#113f29] text-white font-semibold text-sm rounded-lg  -[#164e33]/10 transition-all disabled:opacity-50 active:scale-95 flex items-center gap-2"
                     >
