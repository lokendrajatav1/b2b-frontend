        ...
        <div className="flex flex-wrap items-center gap-4">
           ...
           <div className="flex p-1 bg-gray-100 rounded-xl overflow-hidden">
              {['PENDING', 'VERIFIED', 'ALL'].map((tab) => (
                 <button 
                   ...
                   className={`px-4 py-2 text-base font-semibold uppercase  transition-all rounded-lg ${activeTab === tab ? 'bg-white text-[#164e33] shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                 >
           ...
           <div className="relative">
              ...
              <select 
                ...
                className="pl-10 pr-8 py-2.5 bg-white border border-gray-200 rounded-xl text-base font-semibold appearance-none focus:border-[#164e33] transition-all cursor-pointer min-w-[160px] text-slate-800 outline-none shadow-sm"
              >
                            ...
                            <td className="px-6 py-4 text-right">
                               <div className="flex items-center justify-end gap-3">
                                  <button 
                                    ...
                                    className="px-4 py-2 bg-white border border-gray-200 text-slate-600 rounded-xl font-semibold text-base uppercase  hover:bg-[#164e33] hover:text-white transition-all shadow-sm flex items-center gap-2"
                                  >
                                    Review
          ...
          <>
            ...
            >
               <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between bg-white shrink-0">
                  ...
                  <button onClick={() => setIsModalOpen(false)} className="p-2.5 hover:bg-gray-50 border border-transparent hover:border-gray-200 rounded-xl transition-colors text-slate-500 hover:text-slate-900">
               ...
               <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 space-y-8 sm:space-y-12">
                  <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-center sm:items-start text-center sm:text-left">
                     <div className="w-24 h-24 sm:w-28 sm:h-28 mx-auto sm:mx-0 bg-[#164e33]/10 rounded-xl border border-[#164e33]/20 overflow-hidden shadow-sm shrink-0 flex items-center justify-center text-4xl font-semibold text-[#164e33]">
                     ...
                     <div className="space-y-4 pt-1 flex-1">
                        <div>
                           <h3 className="text-2xl sm:text-3xl font-semibold text-[#164e33] leading-tight capitalize">{selectedVendor.name}</h3>
                           <p className="text-base font-medium text-slate-700 mt-1 flex items-center justify-center sm:justify-start gap-2">
                             <Mail className="w-4 h-4 text-[#164e33]/60" /> {selectedVendor.email}
                           </p>
                        </div>
                        <div className="flex gap-2 justify-center sm:justify-start">
                           <span className="px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-xl text-base font-semibold text-slate-800 uppercase ">{selectedVendor.city || 'Location Pending'}</span>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     ...
                     <div className="space-y-8">
                        <section className="space-y-4">
                           ...
                           <div className="space-y-3">
                              <div className="p-5 bg-gray-50/80 rounded-xl border border-gray-100 flex items-center justify-between group/input transition-all hover:bg-white hover:border-[#164e33]/30">
                                 <div>
                              ...
                              {(selectedVendor.logoUrl || selectedVendor.logo) && (
                                <div className="p-5 bg-gray-50/80 rounded-xl border border-gray-100 flex items-center justify-between group/input transition-all hover:bg-white hover:border-[#164e33]/30">
                                   <div>
                                      <p className="text-base font-semibold text-slate-700 uppercase  mb-1 flex items-center gap-1.5">
                                         <ShieldCheck className="w-4 h-4 text-[#164e33]" /> Company Logo
                                      </p>
                           ...
                           <div className="flex flex-wrap gap-2 pt-1">
                              {selectedVendor.categories && selectedVendor.categories.length > 0 ? (
                                 selectedVendor.categories.map((cat: any) => (
                                    <span key={cat.id} className="px-3 py-1.5 bg-[#164e33]/10 text-[#164e33] border border-[#164e33]/20 rounded-xl text-base font-semibold uppercase ">
                                       {cat.name}
                                    </span>
                                 ))
                              ) : (
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-4 shrink-0">
                     <button 
                        ...
                        className="flex-1 bg-[#164e33] hover:bg-[#113f29] text-white py-4 rounded-xl font-bold text-base uppercase  flex items-center justify-center gap-2 shadow-lg shadow-[#164e33]/20 transition-all disabled:opacity-50"
                     >
                     ...
                     <button 
                        ...
                        disabled={processingId === selectedVendor.id}
                        className="flex-1 bg-white border border-red-200 text-red-600 hover:bg-red-50 py-4 rounded-xl font-bold text-base uppercase  flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                     >
              ...
              >
                 ...
                 <div className="flex flex-col gap-3">
                    <button 
                       ...
                       className="w-full py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold uppercase  shadow-lg shadow-red-500/20"
                    >
                       Confirm Decline
                    </button>
                    <button 
                       onClick={() => setIsRejectConfirmOpen(false)}
                       className="w-full py-4 bg-gray-50 text-slate-700 rounded-xl font-bold uppercase  hover:bg-gray-100"
                    >
                       Cancel
                    </button>
                 </div>
