    ...
    <div className="space-y-8 animate-simple-fade pb-20 p-2 md:p-0">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-gray-100 mb-8 max-w-4xl mx-auto">
        <div>
          ...
          <p className="text-slate-800 font-medium mt-1 text-sm">Updates on your products, services, and buyer inquiries.</p>
        </div>
        ...
        {notifications.length > 0 ? (
          <AnimatePresence>
            {notifications.map((n) => (
              ...
              >
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border ${n.isRead ? 'bg-gray-50 text-slate-800 border-gray-100' : 'bg-white text-[#164e33] border-[#164e33]/10'}`}>
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-slate-800 flex items-center gap-1.5 ml-4">
                      <Clock className="w-3.5 h-3.5" />
                      {new Date(n.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <p className={`text-sm ${n.isRead ? 'text-slate-800' : 'text-slate-800'} leading-relaxed max-w-2xl font-medium`}>{n.message}</p>
                </div>
          ...
          <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 text-slate-800 border border-gray-100 ">
                <Bell className="w-8 h-8" />
            </div>
            <p className="text-slate-800 font-semibold text-sm">No active notifications</p>
            <p className="text-slate-800 font-medium text-sm mt-1">You're all caught up!</p>
          </div>
        )}
      </div>
    </div>
